...
exports.getUsersArrayByName = (req, res) => {
    ...
    if (!name || name.length === 0) {
        return error_handler_1.errorHandler(req, res, 'Name must not be empty');
    }
...
exports.checkUserProfile = (req, res, next) => {
    ...
    if (userId != req.params.id) {
        return error_handler_1.errorHandler(req, res, 'You cannot modify another user\'s profile');
    }
    next();
};
